...
func CryptoTest(t *testing.T) { TestingT(t) }
...
func (s *CryptoSuite) TestCreateKeyBlock(c *C) {
	...
	kb, _ := newKeyBlock(identity, psk, cr, sr)
...
func (s *CryptoSuite) TestEncryption(c *C) {
	...
	data := randomBytes(50)
	aad := newAad(5, 10, 1, 26)
	cipherText, err := dataEncrypt(data, nonce, key, aad, "x")
	c.Assert(err, IsNil)
	...
	clearText, err := dataDecrypt(cipherText, nonce, key, aad, "x")
	c.Assert(err, IsNil)
...
func (s *CryptoSuite) TestEncryptionFailures(c *C) {
	...
	cipherText, err := dataEncrypt(data, nonce, key[:12], aad, "x")
	c.Assert(err, NotNil)

	_, err = dataDecrypt(cipherText, nonce, key[:12], aad, "x")
	c.Assert(err, NotNil)

	cipherText, err = dataEncrypt(data, nonce[:4], key, aad, "x")
	c.Assert(err, NotNil)

	_, err = dataDecrypt(cipherText, nonce[:4], key, aad, "x")
	c.Assert(err, NotNil)
